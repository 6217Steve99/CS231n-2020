...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i, layer_dim in enumerate(hidden_dims):
            self.params['W{}'.format(i+1)] = weight_scale * np.random.randn(layer_input_dim, layer_dim)
            self.params['b{}'.format(i+1)] = weight_scale * np.zeros(layer_dim)

            if self.normalization in ["batchnorm", "layernorm"]:
                self.params['gamma{}'.format(i+1)] = np.ones(layer_dim)
                self.params['beta{}'.format(i+1)] = np.zeros(layer_dim)
